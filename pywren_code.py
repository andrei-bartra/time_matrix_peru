...
'''
 ...
 |                                                                       |
 | Time Matrix Pywren Implementation                                     |
...
def get_costs_lam(array, df):
    df['cost'] = df[['pixel_x_x', 'pixel_y_x', 'pixel_x_y', 'pixel_y_y']]. \
    apply(lambda x:  travel_time(x[0], x[1], x[2], x[3], array), axis=1)
    return df

@profile
def run_pywren(query, array, n_batches):

    query['row_id'] = np.arange(len(query))
    query['batches'] = n_batches*query['row_id']/len(query)
    query['batches'] = query['batches'].apply(np.floor).astype(int)

    pwex = pywren.default_executor()
    batches = [query.loc[query.batches==i] for i in range(n_batches)]
    futures = pwex.map(lambda x: get_costs_lam(array, x), batches)
